...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("NVLS_enable",
				...
				MakeUintegerChecker<uint32_t>())
		.AddAttribute("UB_mesh_enable",
				"UB mesh topology enable flag",
				UintegerValue(0),
				MakeUintegerAccessor(&RdmaHw::enable_ub_mesh),
				MakeUintegerChecker<uint32_t>())
		;
	return tid;
}
...
RdmaHw::RdmaHw(){
	m_total_pause_times = 0;
	m_paused_times = 0;
	enable_pcie_pause = false;
	nvls_enable = 0;
	enable_ub_mesh = 0;
	// Default to 1 to avoid division by zero / unintended grouping before attribute set
	m_gpus_per_server = 1;
}
...
void RdmaHw::disable_ub_mesh_topology() {
	enable_ub_mesh = 0;
}
...
uint32_t RdmaHw::GetNicIdxOfQp(Ptr<RdmaQueuePair> qp){
	...
	uint32_t gpus_per_srv = m_gpus_per_server ? m_gpus_per_server : 1;
	bool same_server = (src / gpus_per_srv == dst / gpus_per_srv);

	// Only use NVSwitch table if it exists AND has entries.
	auto it_nx = m_rtTable_nxthop_nvswitch.find(dip);
	if (same_server && it_nx != m_rtTable_nxthop_nvswitch.end() && !it_nx->second.empty()) {
		auto &v = it_nx->second;
		return v[qp->GetHash() % v.size()];
	}
	// Otherwise use standard routing table.
	auto it = m_rtTable.find(dip);
	if (it != m_rtTable.end() && !it->second.empty()) {
		auto &v = it->second;
		return v[qp->GetHash() % v.size()];
	}
	// Retry NVSwitch table if it appeared later.
	if (it_nx != m_rtTable_nxthop_nvswitch.end() && !it_nx->second.empty()) {
		auto &v = it_nx->second;
		return v[qp->GetHash() % v.size()];
	}
	std::ostringstream oss;
	oss << "Routing tables empty for dip=" << dip
	    << " src=" << src << " dst=" << dst
	    << " m_gpus_per_server=" << m_gpus_per_server
	    << " same_server=" << same_server
	    << " rtTable.size=" << (it==m_rtTable.end()?0:it->second.size())
	    << " rtTable_nxthop_nvswitch.size=" << (it_nx==m_rtTable_nxthop_nvswitch.end()?0:it_nx->second.size())
	    << "; ensure SetRoutingEntries ran before QP creation or adjust GPUsPerServer.";
...
uint32_t RdmaHw::GetNicIdxOfRxQp(Ptr<RdmaRxQueuePair> q){
	...
	// Retry NVSwitch table if population order races.
	if (it_nx != m_rtTable_nxthop_nvswitch.end() && !it_nx->second.empty()) {
		auto &v = it_nx->second;
		return v[q->GetHash() % v.size()];
	}
	std::ostringstream oss;
	oss << "Rx routing tables empty for dip=" << dip
	    << " rtTable.size=" << (it==m_rtTable.end()?0:it->second.size())
	    << " rtTable_nxthop_nvswitch.size=" << (it_nx==m_rtTable_nxthop_nvswitch.end()?0:it_nx->second.size());
	NS_ASSERT_MSG(false, oss.str().c_str());
}
...
void RdmaHw::DumpRoutingTables() {
	std::cout << "[RdmaHw::DumpRoutingTables] Node " << (m_node? (int)m_node->GetId(): -1)
			  << " GPUsPerServer=" << m_gpus_per_server
			  << " nvls_enable=" << nvls_enable
			  << " enable_ub_mesh=" << enable_ub_mesh << std::endl;
	auto dumpOne = [&](const char* name, const std::unordered_map<uint32_t,std::vector<int>>& tbl){
		std::cout << "  Table " << name << " entries=" << tbl.size() << std::endl;
		int shown=0;
		for (auto &e : tbl) {
			uint32_t dip = e.first;
			std::cout << "    dip=" << dip << " ports:";
			for (auto p : e.second) std::cout << " " << p;
			std::cout << std::endl;
			if (++shown >= 64) { // avoid huge spam
				std::cout << "    ... (truncated)" << std::endl; break;
			}
		}
	};
	dumpOne("host", m_rtTable);
	dumpOne("nvswitch", m_rtTable_nxthop_nvswitch);
	if (m_rtTable.empty() && m_rtTable_nxthop_nvswitch.empty()) {
		std::cout << "  WARNING: Both routing tables empty." << std::endl;
	}
}
